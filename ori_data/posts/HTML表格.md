...
- [html表格](#html表格)
  ...
  - [html表格高级特性和可访问性](#html表格高级特性和可访问性)
    - [caption添加表格标题](#caption添加表格标题)
    - [添加表格结构](#添加表格结构)
    - [嵌套表格](#嵌套表格)
    - [表格的可读性优化](#表格的可读性优化)
      - [1.使用行和列的标题](#使用行和列的标题)
...
![a unsuitable table](2.png)

<br>

为了解决这个问题，表格中的标题和单元格有 `colspan`(宽) 和 `rowspan`(高) 属性，这两个属性可以帮助我们实现需要的效果。<br>
...
![table with style](3.png)

显然，col的意义是列，在colgroup中定义所有列的样式，没有样式则`<col>`，有样式就可以在col元素里定义，`<col style="background-color: yellow">，就像上文代码中一样。

<br>

当然，有一种情况使用这种写法依旧比较复杂，即把样式信息应用到每一列。<br>
...
### 添加表格结构
由于表格在有些时候会变得比较复杂，我们可以将表格定义的更加结构化。<br>
...
#### scope属性
`scope`属性可以添加在 th 元素中，用来帮助屏幕阅读设备更好的理解那些标题单元格，这个标题单元格是列标题还是行标题。
```html
<thead>
  <tr>
